 ...
 */
 ...
 function insertPostCodeLine(ln)
 {
        ...
        a.innerHTML = "<input type='tex' name='postcode[]' id='postcode' + ln + "' value='' title='' class='form-text required fluid' />";
        b.innerHTML = "<input type='text name='price[]' id='price' + ln + "' value='' title='' class='form-text required fluid' />";
        c.innerHTML = "<input type='hidden' name='code_selected[]' id='code_deleted" + ln + "' value='0'><input type='button' class='form-submit' value='Remove Row' tabindex='116' onclick='markPostCodeLineDeleted(" + ln + ")'/><b>";
...
function markPostCodeLineDeleted(ln) 
{
        if (confirm("Are you sure you want to remove this line?"))
        {
            ...
            document.getElementById('code_selected ' + ln).value = '1';
            prodLine--;
        }
}