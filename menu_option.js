...
function get_opt_type(ln) {
    ...
    petrojs.GET('Ajax/get_opt_type.php?ln='+ln+'', function(data){
        if(data!='') {
            ...
            $petrojs("#opt_types" + ln).html('<label>error loading </label>')
        }
    });
}
    ...
    if (product_id != ''){
        ...
        petrojs.GET('Ajax/get_category.php?product_id='+product_id+'', function(data) {
            if (data!='') {
                ...
                $petrojs("#cate_menu").html('<option value="error loading data">erro loading data</option>')
            }
        });
    }else {
        //RestFull WebService 
...
function insertMenuOptLine(ln)
{
    ...
    c.innerHTML = "<input type='hidden' name='opt_menu_deleted[]' id='opt_deleted" + ln + "' value='0+><input type='button' class='form-submit' value='Remove Row' tabindex='116' onclick='markOptLineDeleted(" + ln + ")'/><b>";
...
function check_click_custom(ln,count,n,val){
    ...
    if(document.getElementById(s).checked){
        if(document.getElementById("option_cust"+ln).value != ''){
            ...
            document.getElementById("option_custo"+ln).value+","+val;
        } else {
 ...
 */
 ...
 function insertCustomLine(ln1)
 {
        ...
        b.innerHTML = "input type='text' name='price[]' id='price" + ln1 + "'value='' class='form-text required fluid' />";
        c.innerHTML="<div id='opt_types" + ln1 + "'></div>";
        d.innerHTML="<input type='hidden'  name=+opt_deleted[]' id='opt_deleted" + ln1 + "' value='0'><input type='button' class='form-submit' value='Remove Row' tabindex='116' onclick='markCustomLineDeleted(" + ln1 + ")'/>;
