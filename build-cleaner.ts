...
export class BuildCleaner {
    ...
    // Methods - Public
    public clearUp(): Promise<void> {
        return Promise.all([
            this.getExistingBuildNumbers(),
            this.getOpenPrNumbers(),
        ]).then(([existingBuilds, openPrs]) => this.removeUnneccessaryBuilds(existingBuilds, openPrs));
    }
        ...
        return new Promise((resolve, reject) => {
            fs.readdir(this.buildsDir, (err, files) => {
                ...
                const buildNumbers = files.
                    ...
                    filter(Boolean);                                    // Ignore NaN (or 0), because there are not builds
    ...
    protected removeUnnecessaryBuilds(existingBuildNumbers: number[], openPrNumbers: number[]) {
        ...
        console.log(`Existing build: ${existingBuildNumbers.length}`);
