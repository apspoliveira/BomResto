...
function insertOptLine(ln)
{
    ...
    a.innerHTML = "<input type='text' name='titme[]' id='title' + ln + "' value='' title='' class='formtext required fluid' />";
    ...
    d.innerHTML="<input type='hidden' name='opt_selected[]' id='opt_deleted[]' id='opt_selected" + ln + "' value='0'><input type='button' class='form-submit' value='Remove Row' tabindex='116' onclick='markOptLineDeleted(" + ln + ")'/><b>";
...
function markOptLineDeleted(ln)
{
    ...
    //{
            // collapse product line; updated deleted value 
            document.getElementById('opt_line' + ln).style.display = 'none';
            document.getElementById('opt_selected' + ln).value = '1';
            prodLine--;
    //}
}

function option_add(ln){
    var obj_type = document.getElementById("opt_type"+ln).value;
    if(opt_type ==0){
